...
def get_stat(name):
	...
	if ret:
		if name.startswith('diskstat_'):
			...
			# MODIFIED: removed dev from metric name
			# dev = name[fir+1:sec]
			# MODIFIED: collect metrics only for one device
			# label = name[sec+1:]
			label = name[fir+1:]

			try:
				# MODIFIED: aggregate values from all partitions
				# return stats[dev][label]
				stats_sum = 0.0
				for dev in PARTITIONS:
					stats_sum += stats[dev][label]
				return stats_sum / float(len(PARTITIONS))
			except:
